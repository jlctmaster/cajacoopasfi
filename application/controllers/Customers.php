...
class Customers extends Persons
{
	...
	*/
	public function view($customer_id = -1)
	{
		...
		if(empty($info->person_id) || empty($info->date) || empty($info->user_id))
		{
			$data['person_info']->date = date('Y-m-d H:i:s');
			$data['person_info']->user_id = $this->User->get_logged_in_user_info()->person_id;
		}

		$user_info = $this->User->get_info($info->user_id);
	...
	*/
	public function save($customer_id = -1)
	{
		...
		$this->Customer->set_log("ID: ".$customer_id);
		...
		$customer_data = array(
			...
			'ruc' => $this->input->post('ruc') == '' ? NULL : $this->input->post('ruc'),
