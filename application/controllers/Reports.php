...
class Reports extends Secure_Controller
{
	public function __construct()
	{
		...
		if(sizeof($exploder) > 1)
		{
			...
			if(!$this->User->has_grant('reports_' . $submodule_id, $this->User->get_logged_in_user_info()->person_id))
			{
	...
	{
		$data['grants'] = $this->xss_clean($this->User->get_user_grants($this->session->userdata('person_id')));
	...
	public function summary_users($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);

		$this->load->model('reports/Summary_users');
		$model = $this->Summary_users;
		...
		$data = array(
			'title' => $this->lang->line('reports_users_summary_report'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => $summary
	...
	//Graphical summary users report
	public function graphical_summary_users($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);
		...
		foreach($report_data as $row)
		{
			...
			$labels[] = $row['user'];
			$series[] = array('meta' => $row['user'] . ' ' . round($row['total'] / $summary['total'] * 100, 2) . '%', 'value' => $row['total']);
		}
	...
	public function specific_user_input()
	{
		$data = array();
		$data['specific_input_name'] = $this->lang->line('reports_user');

		$users = array();
		foreach($this->User->get_all()->result() as $user)
		{
			$users[$user->person_id] = $this->xss_clean($user->first_name . ' ' . $user->last_name);
		}
		$data['specific_input_data'] = $users;
		$data['sale_type_options'] = $this->get_sale_type_options();

		$this->load->view('reports/specific_input', $data);
	}

	public function specific_user($start_date, $end_date, $user_id, $sale_type)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'user_id' => $user_id, 'sale_type' => $sale_type);

		$this->load->model('reports/Specific_user');
		$model = $this->Specific_user;
		...
		$data = array(
			'title' => $this->xss_clean($user_info->first_name . ' ' . $user_info->last_name . ' ' . $this->lang->line('reports_report')),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $headers,
			'editable' => 'sales',
			'summary_data' => $summary_data,
