...
class Items extends Secure_Controller
{
	...
	*/
	...
	public function suggest_by_location()
	{
		$suggestions = $this->xss_clean($this->Item->get_search_suggestions($this->input->post_get('term'),
			array('search_custom' => FALSE, 'is_deleted' => FALSE, 'stock_location_id' => $this->input->post_get('location_id')), TRUE));
	...
	*/
	...
	Gives search suggestions based on what is being searched for
	*/
	public function suggest_mark()
	{
		$suggestions = $this->xss_clean($this->Item->get_mark_suggestions($this->input->get('term')));
	...
	*/
	...
	public function get_item_info($item_ids,$location_id = 0)
	{
		$stock_location_id = ($location_id!=0 ? $location_id : $this->item_lib->get_item_location());
		$item_infos = $this->Item->get_multiple_info(explode(":", $item_ids), $stock_location_id);
		$data_row = $item_infos->result_array();
	...
	public function view($item_id = -1)
	{
		...
		$data['item_type_disabled'] = !$this->User->has_grant('item_kits', $this->User->get_logged_in_user_info()->person_id);
		...
		$uom = array('' => $this->lang->line('items_none'));
		foreach($this->Uom->get_all()->result_array() as $row)
		{
			$uom[$this->xss_clean($row['uom_id'])] = $this->xss_clean($row['name']);
		}
		$data['uom'] = $uom;
		$data['selected_uom'] = $item_info->uom_id;
	...
	public function bulk_edit()
	{
		...
		$data['bulk_choices'] = array(
			'' => $this->lang->line('items_do_nothing'), 
			1  => $this->lang->line('items_change_all_to_bulk'),
			0  => $this->lang->line('items_change_all_to_unbulk'));
	...
	public function save($item_id = -1)
	{
		...
		$item_data = array(
			...
			'is_bulk' => $this->input->post('is_bulk') != NULL,
		...
		if($this->Item->save($item_data, $item_id))
		{
			...
			if(!$use_destination_based_tax)
			{
				...
				$this->Item->set_log($this->db->last_query());
			}
			...
			if(($handle = fopen($_FILES['file_path']['tmp_name'], 'r')) !== FALSE)
			{
				...
				while(($data = fgetcsv($handle)) !== FALSE)
				{
					...
					if(!$invalidated && $this->Item->save($item_data))
					{
						...
						$User_id = $this->User->get_logged_in_User_info()->person_id;
						$emp_info = $this->User->get_info($User_id);
