...
function current_language_code($load_system_language = FALSE)
{
	...
	// Returns the language code of the user if set or system language code if not
	if($user->is_logged_in() && $load_system_language != TRUE)
	{
		$user_info = $user->get_logged_in_user_info();

		if(property_exists($user_info, 'language_code') && !empty($user_info->language_code))
		{
			return $user_info->language_code;
		}
	}
...
function current_language($load_system_language = FALSE)
{
	...
	// Returns the language of the user if set or system language if not
	if($user->is_logged_in() && $load_system_language != TRUE)
	{
		$user_info = $user->get_logged_in_user_info();
		if(property_exists($user_info, 'language') && !empty($user_info->language))
		{
