...
<?php echo form_open('overall_cashs/save_cost/'.$cost_info->cost_id, array('id'=>'cost_edit_form', 'class'=>'form-horizontal')); ?>
	<fieldset id="costs">
		<div class="form-group form-group-sm">	
			...
			<div class='col-xs-6'>
				...
				<div class='col-xs-6'>
					<?php if($this->config->item('cost_overallcash_number_automatic') == '1')
					{
		...
		<div class="form-group form-group-sm">
			...
			<div class='col-xs-4'>
				<?php echo form_input(array(
						...
						'type' => 'number',
						'step' => 0.01,
						'max' => (!empty($cost_info->currency) ? ($cost_info->currency == CURRENCY ? $overall_cash_info->endingbalance : $overall_cash_info->usdendingbalance) : $overall_cash_info->endingbalance ),
						'value'=>$cost_info->amount)
						);?>
				<?php echo form_input(array(
					'name'=>'overall_cash_id',
					'id'=>'overall_cash_id',
					'class'=>'form-control input-sm',
					'type' => 'hidden',
					'value'=>$overall_cash_info->overall_cash_id)
					);?>
				<?php echo form_input(array(
					'name'=>'cash_balance',
					'id'=>'cash_balance',
...
$(document).ready(function()
{
	...
	$('#cost_edit_form').validate($.extend({
		...
		rules:
		{
			<?php if($this->config->item('cost_overallcash_number_automatic') == '0'):?>
				documentno: 'required',
			<?php endif;?>
