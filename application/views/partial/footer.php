		</div>
	</div>

	<div id="footer">
		<div class="jumbotron push-spaces">
			<!--<strong><?php echo $this->lang->line('common_you_are_using_ospos'); ?>
  			<?php echo $this->config->item('application_version'); ?> - <?php echo substr('$Id: 4f5ad5761503bbc94152cbf16afa76439ca155b8 $', 5, 7); ?></strong>.
			<?php echo $this->lang->line('common_please_visit_my'); ?>
			<a href="https://presionaclic.com/" target="_blank"><?php echo $this->lang->line('common_website'); ?></a>
			<?php echo $this->lang->line('common_learn_about_project'); ?>-->
			<strong><?php echo $this->lang->line('common_copyright'); ?></strong>
			<?php echo $this->lang->line('common_powered_by'); ?>: 
			<a href="https://presionaclic.com/" target="_blank"><?php echo $this->lang->line('common_vendor'); ?></a>
		</div>
	</div>
</body>
</html>
