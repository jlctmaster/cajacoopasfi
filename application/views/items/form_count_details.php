...
<table id="items_count_details" class="table table-striped table-hover">
	<thead>
		...
		<tr>
			...
			<th width="20%"><?php echo $this->lang->line('items_inventory_user'); ?></th>
	...
	<tbody id="inventory_result">
		...
		*/
		...
		{
			$user = $this->User->get_info($row['trans_user']);
			array_push($user_name, $user->first_name . ' ' . $user->last_name);
		}
		?>
	</tbody>
</table>
...
function display_stock(location_id)
{
    ...
    var user_data = <?php echo json_encode($user_name); ?>;
    ...
    for (var index = 0; index < inventory_data.length; index++)
    {
        ...
        if(data['trans_location'] == location_id)
        {
            ...
            td.appendChild(document.createTextNode(user_data[index]));
            tr.appendChild(td);
