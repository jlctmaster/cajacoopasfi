...
class Convert_uom extends CI_Model
{
	/**
	...
	{
		$this->db->from('uom_conversions');
		$this->db->where('uom_conversion_id', $id);
	...
	{
		$query = $this->db->get_where('uom_conversions', array('uom_conversion_id' => $id), 1);
		...
		else
		{
			...
			foreach($this->db->list_fields('uom_conversions') as $field)
			{
				$uom_obj->$field = '';
			}
	...
	public function save($uom_data, $uom_id = FALSE)
	{
		...
		{
			if($this->db->insert('uom_conversions', $uom_data))
			{
                            
				$uom_data['uom_conversion_id'] = $this->db->insert_id();
		...
		$this->db->where('uom_conversion_id', $uom_id);

		return $this->db->update('uom_conversions', $uom_data);
	}
	...
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'name', $order='asc', $count_only = FALSE)
	{
		...
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(uom_conversion_id) as count');
		}
