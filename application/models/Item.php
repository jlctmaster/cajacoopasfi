...
class Item extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			...
			$this->db->select('MAX(uom.name) AS uom');
			$this->db->select('MAX(items.category) AS category');
			$this->db->select('MAX(items.mark) AS mark');
			...
			$this->db->select('MAX(items.is_bulk) AS is_bulk');
		...
		$this->db->join('uom AS uom', 'uom.uom_id = items.uom_id');
		...
		$attributes_enabled = false; //count($filters['definition_ids']) > 0;

		if(!empty($search))
		{
			$this->db->group_start();
				$this->db->like('items.name', $search);
				$this->db->or_like('uom.name', $search);
				...
				$this->db->or_like('items.mark', $search);
	...
	*/
	...
	public function get_quantity_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE, 'stock_location_id' => 'all'), $unique = FALSE, $limit = 25)
	{
		$suggestions = array();
		$non_kit = array(ITEM, ITEM_AMOUNT_ENTRY);
		...
		$this->db->where('deleted', $filters['is_deleted']);
		if(!empty($filters['stock_location_id']) && $filters['stock_location_id'] != 'all')
		{
			$this->db->where('location_id', $filters['stock_location_id']);
		}
		$this->db->where_in('item_type', $non_kit); // standard, exclude kit items since kits will be picked up later
	...
	public function get_mark_suggestions($search)
	{
		$suggestions = array();
		$this->db->distinct();
		$this->db->select('mark');
		$this->db->from('items');
		$this->db->like('mark', $search);
		$this->db->where('deleted', 0);
		$this->db->order_by('mark', 'asc');
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('label' => $row->mark);
		}
