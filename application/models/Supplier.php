<?php if (!defined('BASEPATH')) exit('No direct script access allowed');

/**
 * Supplier class
 */

class Supplier extends Person
{	

	private $log;

	function get_log(){
		return $this->log;
	}

	function set_log($log){
		return $this->log = $this->log."<br>".$log;
	}

	/*
	Determines if a given person_id is a customer
	*/
	public function exists($person_id)
	{
		$this->db->from('suppliers AS suppliers');	
		$this->db->join('people AS people', 'people.person_id = suppliers.person_id');
		$this->db->where('suppliers.person_id', $person_id);
		
		return ($this->db->get()->num_rows() == 1);
	}

	/*
	Gets total of rows
	*/
	public function get_total_rows()
	{
		$this->db->from('suppliers');
		$this->db->where('deleted', 0);

		return $this->db->count_all_results();
	}
	
	/*
	Returns all the suppliers
	*/
	public function get_all($limit_from = 0, $rows = 0)
	{
		$this->db->select('
			suppliers.*,
			people.*,
			growing_areas.name AS growing_area_name
		');
		$this->db->from('suppliers AS suppliers');
		$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
		$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
		$this->db->where('suppliers.deleted', 0);
		$this->db->order_by('suppliers.company_name', 'asc');
		if($rows > 0)
		{
			$this->db->limit($rows, $limit_from);
		}

		return $this->db->get();		
	}
	
	/*
	Gets information about a particular supplier
	*/
	public function get_info($supplier_id)
	{
		$this->db->select('
			suppliers.*,
			people.*,
			growing_areas.name AS growing_area_name
		');
		$this->db->from('suppliers AS suppliers');	
		$this->db->join('people AS people', 'people.person_id = suppliers.person_id');
		$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
		$this->db->where('suppliers.person_id', $supplier_id);
		$query = $this->db->get();

		if($query->num_rows() == 1)
		{
			return $query->row();
		}
		else
		{
			//Get empty base parent object, as $supplier_id is NOT an supplier
			$person_obj = parent::get_info(-1);
			
			//Get all the fields from supplier table		
			//append those fields to base parent object, we we have a complete empty object
			foreach($this->db->list_fields('suppliers') as $field)
			{
				$person_obj->$field = '';
			}
			
			return $person_obj;
		}
	}
	
	/*
	Gets information about multiple suppliers
	*/
	public function get_multiple_info($suppliers_ids)
	{
		$this->db->select('
			suppliers.*,
			people.*,
			growing_areas.name AS growing_area_name
		');
		$this->db->from('suppliers AS suppliers');
		$this->db->join('people AS people', 'people.person_id = suppliers.person_id');
		$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
		$this->db->where_in('suppliers.person_id', $suppliers_ids);
		$this->db->order_by('people.last_name', 'asc');

		return $this->db->get();
	}
	
	/*
	Inserts or updates a suppliers
	*/
	public function save_supplier(&$person_data, &$supplier_data, $supplier_id = FALSE)
	{
		$success = FALSE;

		//Run these queries as a transaction, we want to make sure we do all or nothing
		$this->db->trans_start();

		$this->set_log("ID To Send: ".$supplier_id);
		
		if(parent::save($person_data,$supplier_id))
		{
			$this->set_log($this->db->last_query());

			if(!$supplier_id || !$this->exists($supplier_id))
			{
				$this->set_log($this->db->last_query());
				$supplier_data['person_id'] = (!empty($person_data['person_id']) ? $person_data['person_id'] : $supplier_id);
				$success = $this->db->insert('suppliers', $supplier_data);
				$this->set_log($this->db->last_query());
			}
			else
			{
				$this->db->where('person_id', $supplier_id);
				$success = $this->db->update('suppliers', $supplier_data);
				$this->set_log($this->db->last_query());
			}
		}else{
			$this->set_log(parent::get_log());

			$this->set_log($this->db->last_query());	
		}
		
		$this->db->trans_complete();
		
		$success &= $this->db->trans_status();

		return $success;
	}
	
	/*
	Deletes one supplier
	*/
	public function delete($supplier_id)
	{
		$this->db->where('person_id', $supplier_id);

		return $this->db->update('suppliers', array('deleted' => 1));
	}
	
	/*
	Deletes a list of suppliers
	*/
	public function delete_list($supplier_ids)
	{
		$this->db->where_in('person_id', $supplier_ids);

		return $this->db->update('suppliers', array('deleted' => 1));
 	}
 	
 	/*
	Get search suggestions to find suppliers
	*/
	public function get_search_suggestions($search, $unique = FALSE, $limit = 25)
	{
		$suggestions = array();

		$this->db->from('suppliers AS suppliers');
		$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
		$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
		$this->db->group_start();
			$this->db->like('people.first_name', $search);
			$this->db->or_like('people.last_name', $search); 
			$this->db->or_like('people.dni', $search); 
			$this->db->or_like('suppliers.ruc', $search); 
			$this->db->or_like('CONCAT(people.first_name, " ",people.last_name)', $search);
		$this->db->group_end();
		$this->db->where('suppliers.deleted', 0);
		$this->db->order_by('people.last_name', 'asc');
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('value' => $row->person_id, 'label' => $row->first_name . ' ' . $row->last_name);
		}

		if(!$unique)
		{

			$this->db->from('suppliers AS suppliers');
			$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
			$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
			$this->db->where('suppliers.deleted', 0);
			$this->db->like('suppliers.company_name', $search);
			$this->db->order_by('suppliers.company_name', 'asc');
			foreach($this->db->get()->result() as $row)
			{
				$suggestions[] = array('value' => $row->person_id, 'label' => $row->company_name);
			}

			$this->db->from('suppliers AS suppliers');
			$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
			$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
			$this->db->where('suppliers.deleted', 0);
			$this->db->distinct();
			$this->db->like('suppliers.agency_name', $search);
			$this->db->where('suppliers.agency_name IS NOT NULL');
			$this->db->order_by('suppliers.agency_name', 'asc');
			foreach($this->db->get()->result() as $row)
			{
				$suggestions[] = array('value' => $row->person_id, 'label' => $row->agency_name);
			}
			
			$this->db->from('suppliers AS suppliers');
			$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
			$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
			$this->db->where('suppliers.deleted', 0);
			$this->db->like('people.email', $search);
			$this->db->order_by('people.email', 'asc');
			foreach($this->db->get()->result() as $row)
			{
				$suggestions[] = array('value' => $row->person_id, 'label' => $row->email);
			}

			$this->db->from('suppliers AS suppliers');
			$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
			$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
			$this->db->where('suppliers.deleted', 0);
			$this->db->like('people.phone_number', $search);
			$this->db->order_by('people.phone_number', 'asc');
			foreach($this->db->get()->result() as $row)
			{
				$suggestions[] = array('value' => $row->person_id, 'label' => $row->phone_number);
			}

			$this->db->from('suppliers AS suppliers');
			$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
			$this->db->join('growing_areas AS growing_areas', 'growing_areas.growing_area_id = suppliers.growing_area_id', 'left');
			$this->db->where('suppliers.deleted', 0);
			$this->db->like('suppliers.account_number', $search);
			$this->db->order_by('suppliers.account_number', 'asc');
			foreach($this->db->get()->result() as $row)
			{
				$suggestions[] = array('value' => $row->person_id, 'label' => $row->account_number);
			}
		}

		//only return $limit suggestions
		if(count($suggestions) > $limit)
		{
			$suggestions = array_slice($suggestions, 0, $limit);
		}

		return $suggestions;
	}
 	
 	/*
	Get search suggestions to find suppliers
	*/
	public function get_search_suppliers_suggestions($search, $limit = 25)
	{
		$suggestions = array();

		$this->db->from('suppliers AS suppliers');
		$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
		$this->db->group_start();
			$this->db->like('people.first_name', $search);
			$this->db->or_like('people.last_name', $search); 
			$this->db->or_like('people.dni', $search); 
			$this->db->or_like('suppliers.ruc', $search); 
			$this->db->or_like('suppliers.company_name', $search); 
			$this->db->or_like('CONCAT(people.first_name, " ",people.last_name)', $search);
		$this->db->group_end();
		$this->db->where('suppliers.deleted', 0);
		$this->db->where('suppliers.ispartner', 0);
		$this->db->order_by('people.last_name', 'asc');
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('value' => $row->person_id, 'label' => (!empty($row->company_name) ? $row->company_name : $row->first_name . ' ' . $row->last_name));
		}

		//only return $limit suggestions
		if(count($suggestions) > $limit)
		{
			$suggestions = array_slice($suggestions, 0, $limit);
		}

		return $suggestions;
	}

 	/*
	Gets rows
	*/
	public function get_found_rows($search)
	{
		return $this->search($search, 0, 0, 'last_name', 'asc', TRUE);
	}
	
	/*
	Perform a search on suppliers
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'last_name', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(suppliers.person_id) as count');
		}
		else{
			$this->db->select('
				suppliers.*,
				people.*,
				growing_areas.name AS growing_area_name
			');
		}

		$this->db->from('suppliers AS suppliers');
		$this->db->join('people AS people', 'suppliers.person_id = people.person_id');
		$this->db->join('growing_areas AS growing_areas', 'suppliers.growing_area_id = growing_areas.growing_area_id', 'left');
		$this->db->group_start();
			$this->db->like('people.first_name', $search);
			$this->db->or_like('people.last_name', $search);
			$this->db->or_like('suppliers.company_name', $search);
			$this->db->or_like('suppliers.agency_name', $search);
			$this->db->or_like('growing_areas.name', $search);
			$this->db->or_like('people.email', $search);
			$this->db->or_like('people.phone_number', $search);
			$this->db->or_like('suppliers.account_number', $search);
			$this->db->or_like('CONCAT(people.first_name, " ",people.last_name)', $search);
		$this->db->group_end();
		$this->db->where('suppliers.deleted', 0);
		
		// get_found_rows case
		if($count_only == TRUE)
		{
			return $this->db->get()->row()->count;
		}

		$this->db->order_by($sort, $order);

		if($rows > 0)
		{
			$this->db->limit($rows, $limit_from);
		}

		return $this->db->get();
	}
}
?>
